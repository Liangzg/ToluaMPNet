...
public class UIPanelWrap
{
	...
	static int _CreateUIPanel(IntPtr L)
	{
		...
		if (count == 0)
		{
			UIPanel obj = new UIPanel();
			LuaScriptMgr.Push(L, obj);
			return 1;
		}
		else
		{
			LuaDLL.luaL_error(L, "invalid arguments to method: UIPanel.New");
		}
	...
	static int set_clipTexture(IntPtr L)
	{
		...
		obj.clipTexture = LuaScriptMgr.GetNetObject<Texture2D>(L, 3);
		return 0;
	}
	...
	static int CompareFunc(IntPtr L)
	{
		...
		UIPanel arg0 = LuaScriptMgr.GetNetObject<UIPanel>(L, 1);
		UIPanel arg1 = LuaScriptMgr.GetNetObject<UIPanel>(L, 2);
	...
	{
		LuaScriptMgr.CheckArgsCount(L, 2);
		UIPanel obj = LuaScriptMgr.GetNetObject<UIPanel>(L, 1);
